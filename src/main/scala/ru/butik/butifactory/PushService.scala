...
class PushService(pushHost: String, pushPort: String, pushPath: String, pushKey: String) {
  import com.twitter.conversions.time._
  private val twitter = Http.client
    .withRetryBackoff(Backoff.exponentialJittered(2.seconds, 60.seconds).take(10))
    .withTransport.tls(pushHost)
    .newService(s"$pushHost:$pushPort")

  def pushDevice(deviceId: String, artifactVersion: ArtifactVersionAndroid): Future[Either[String, GoogleMessageResponse]] = {
    val request = http.Request(http.Method.Post, pushPath)
    request.host = pushHost
    request.setContentTypeJson
    request.authorization = s"key=$pushKey"
